...
def calculate_gpu_hours(usage_data):
    ...
    for job in usage_data:
        ...
        if partition not in usage_by_user[user]["partitions"]:
            usage_by_user[user]["partitions"][partition] = {"gpu_hours": 0}

        gpu_hours = hours * gpus
        usage_by_user[user]["partitions"][partition]["gpu_hours"] += gpu_hours
        usage_by_user[user]["accounts"].add(account)
        all_accounts.add(account)

        usage_by_user[user]["total"] += gpu_hours

    return usage_by_user, all_accounts


def format_output(usage_by_user, gpu_limits):
    # Table header
    header = ""
    header += "┌────────────────┬────────────────┬─────────────────────────────────┬──────────────┬──────────────┐\n"
    header += "│      User      │     Account    │        Partition Details        │ Total Usage  │ Usage limit  │\n"
    header += "│                │                │                                 │ (GPU Hours)  │ (GPU Hours)  │\n"
    header += "├────────────────┼────────────────┼─────────────────────────────────┼──────────────┼──────────────┤\n"
    ...
    for user, data in usage_by_user.items():
        ...
        if account in gpu_limits and user in gpu_limits[account]:
            ...
            if user_limit["total"] is not None:
                used_hours = float(
                    user_limit["used"]) / 60 if user_limit["used"] else 0
                total_hours = float(
                    user_limit["total"]) / 60 if user_limit["total"] else 0
                gpu_limit = f"{used_hours:.2f}/{total_hours:.2f}"

        # First line with user, account, first line of partition details, total GPU hours, and GPU limit
        body += f"│ {user:<14} │ {account:<14} │ {partition_lines[0]:<31} │ {data['total']:>12.2f} │ {gpu_limit:>12} │\n"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Calculate Slurm GPU usage")
    parser.add_argument(
    ...
    usage_by_user, all_accounts = calculate_gpu_hours(usage_data)
    ...
    for account in all_accounts:
        ...
        if account_limit and account_limit["total"] is not None:
            used_hours = float(
                account_limit["used"]) / 60 if account_limit["used"] else 0
            total_hours = float(
